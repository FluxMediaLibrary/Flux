sub Main()
    ...
    failures += ExpectEqual("accepts hostname containing s", NormalizeServerUrl("https://server.example/"), "https://server.example")
    ...
    failures += ExpectEqual("encodes URL path component", UrlEncode("show id/1"), "show%20id%2F1")
    ...
    failures += ExpectEqual("HTML error body is invalid JSON", SafeJsonParse("<html><h1>502 Bad Gateway</h1></html>"), invalid)
    requestJson = FormatJson(CanonicalizeRequestJson({ deviceName: "Living room", deviceId: "roku-device-1", appVersion: "1.0.8", capabilities: { supportsHevc: true, maxBitrate: 20000000 } }))
    failures += ExpectEqual("request JSON preserves deviceName", requestJson.InStr("""deviceName""") >= 0, true)
    failures += ExpectEqual("request JSON preserves deviceId", requestJson.InStr("""deviceId""") >= 0, true)
    failures += ExpectEqual("request JSON preserves appVersion", requestJson.InStr("""appVersion""") >= 0, true)
    failures += ExpectEqual("nested request JSON preserves supportsHevc", requestJson.InStr("""supportsHevc""") >= 0, true)
    failures += ExpectEqual("nested request JSON preserves maxBitrate", requestJson.InStr("""maxBitrate""") >= 0, true)
